...
test.serial('tails a simple file', async (t) => {
  ...
  let result = '';
  let exitCode = null;
  const cp = tail(state, undefined, [tmpFileName1]).on('close', (ec: number) => {
    exitCode = ec;
  });
  cp.stdout.on('data', (text: string) => {
...
test.serial('tail skips n bytes', async (t) => {
  ...
  let result = '';
  let exitCode = null;
  const cp = tail(state, undefined, ['-c', `+${skipBytes + 1}`, tmpFileName1]).on('close', (ec: number) => {
    exitCode = ec;
  });
  cp.stdout.on('data', (text: string) => {
    result += text.toString();
...
test.serial('tails with -F', async (t) => {
  ...
  const cp = tail(state, undefined, ['-F', tmpFileName1]).on('exit', () => {
    stillRunning = false;
  });
...
test.serial('prunes tail tasks on state object', async (t) => {
  ...
  t.deepEqual(
    state.ActiveTails.map((t) => t.childProcess),
    [cp1, cp2, cp3]
  );
  ...
  t.deepEqual(
    state.TerminatedTails.map((t) => t.childProcess),
    [cp2]
  );
  t.deepEqual(
    state.ActiveTails.map((t) => t.childProcess),
